...
public class CommandTests
{
    ...
    public void ValidateCommandConverters()
    {
        Assert.Multiple(
            ...
            () =>
            {
                var result = Request.ScanAsync("0").Converter(new object[] { "0", new object[] { (gs)"key1", (gs)"key2" } });
                Assert.Equal("0", result.Item1);
                Assert.Equal(["key1", "key2"], result.Item2.Select(k => k.ToString()).ToArray());
            },
            () =>
            {
                var result = Request.ScanAsync("10").Converter(new object[] { "5", new object[] { (gs)"test" } });
                Assert.Equal("5", result.Item1);
                Assert.Equal(["test"], result.Item2.Select(k => k.ToString()).ToArray());
            },
            () =>
            {
                var result = Request.ScanAsync("0").Converter(new object[] { "0", Array.Empty<object>() });
                Assert.Equal("0", result.Item1);
