...
public class CommandTests
{
    ...
    public void ValidateStreamCommandArgs()
    {
        Assert.Multiple(
            ...
            () => Assert.Equal(["XGROUPCREATE", "key", "group", "$", "MKSTREAM"], Request.StreamCreateConsumerGroup("key", "group", default, true, null).GetArgs()),
            () => Assert.Equal(["XGROUPCREATE", "key", "group", "0"], Request.StreamCreateConsumerGroup("key", "group", "0", false, null).GetArgs()),
            () => Assert.Equal(["XGROUPCREATE", "key", "group", "0", "ENTRIESREAD", "10"], Request.StreamCreateConsumerGroup("key", "group", "0", false, 10).GetArgs()),

            // StreamDeleteConsumerGroup
            () => Assert.Equal(["XGROUPDESTROY", "key", "group"], Request.StreamDeleteConsumerGroup("key", "group").GetArgs()),

            // StreamCreateConsumer
            () => Assert.Equal(["XGROUPCREATECONSUMER", "key", "group", "consumer"], Request.StreamCreateConsumer("key", "group", "consumer").GetArgs()),

            // StreamDeleteConsumer
            () => Assert.Equal(["XGROUPDELCONSUMER", "key", "group", "consumer"], Request.StreamDeleteConsumer("key", "group", "consumer").GetArgs()),

            // StreamConsumerGroupSetPosition
            () => Assert.Equal(["XGROUPSETID", "key", "group", "0-0"], Request.StreamConsumerGroupSetPosition("key", "group", "0-0", null).GetArgs()),
            () => Assert.Equal(["XGROUPSETID", "key", "group", "0-0", "ENTRIESREAD", "5"], Request.StreamConsumerGroupSetPosition("key", "group", "0-0", 5).GetArgs()),
            ...
            () => Assert.Equal(["XPENDING", "key", "group", "-", "+", "10", "consumer"], Request.StreamPendingMessages("key", "group", "-", "+", 10, "consumer", null).GetArgs()),
            () => Assert.Equal(["XPENDING", "key", "group", "IDLE", "1000", "-", "+", "10", "consumer"], Request.StreamPendingMessages("key", "group", "-", "+", 10, "consumer", OneSecond).GetArgs()),

            // StreamClaim
            () => Assert.Equal(["XCLAIM", "key", "group", "consumer", "1000", "1-0"], Request.StreamClaim("key", "group", "consumer", OneSecond, ["1-0"]).GetArgs()),
            () => Assert.Equal(["XCLAIM", "key", "group", "consumer", "1000", "1-0", "IDLE", "500"], Request.StreamClaim("key", "group", "consumer", OneSecond, ["1-0"], new StreamClaimOptions { Idle = TimeSpan.FromMilliseconds(500) }).GetArgs()),
            () => Assert.Equal(["XCLAIM", "key", "group", "consumer", "1000", "1-0", "FORCE"], Request.StreamClaim("key", "group", "consumer", OneSecond, ["1-0"], new StreamClaimOptions { Force = true }).GetArgs()),

            // StreamClaimIdsOnly
            () => Assert.Equal(["XCLAIM", "key", "group", "consumer", "1000", "1-0", "JUSTID"], Request.StreamClaimIdsOnly("key", "group", "consumer", OneSecond, ["1-0"]).GetArgs()),

            // StreamAutoClaim
            () => Assert.Equal(["XAUTOCLAIM", "key", "group", "consumer", "1000", "0-0"], Request.StreamAutoClaim("key", "group", "consumer", OneSecond, "0-0", null).GetArgs()),
            () => Assert.Equal(["XAUTOCLAIM", "key", "group", "consumer", "1000", "0-0", "COUNT", "10"], Request.StreamAutoClaim("key", "group", "consumer", OneSecond, "0-0", 10).GetArgs()),

            // StreamAutoClaimJustId
            () => Assert.Equal(["XAUTOCLAIM", "key", "group", "consumer", "1000", "0-0", "JUSTID"], Request.StreamAutoClaimJustId("key", "group", "consumer", OneSecond, "0-0", null).GetArgs()),

            // StreamInfo
            () => Assert.Equal(["XINFOSTREAM", "key"], Request.StreamInfo("key").GetArgs()),

            // StreamGroupInfo
            () => Assert.Equal(["XINFOGROUPS", "key"], Request.StreamGroupInfo("key").GetArgs()),

            // StreamConsumerInfo
            () => Assert.Equal(["XINFOCONSUMERS", "key", "group"], Request.StreamConsumerInfo("key", "group").GetArgs())
    ...
    public void ValidateStreamCommandConverters()
    {
        Assert.Multiple(
            ...
            // StreamCreateConsumerGroup
            () => Assert.True(Request.StreamCreateConsumerGroup("key", "group", default, true, null).Converter("OK")),

            // StreamDeleteConsumerGroup
            () => Assert.True(Request.StreamDeleteConsumerGroup("key", "group").Converter(true)),
            () => Assert.False(Request.StreamDeleteConsumerGroup("key", "group").Converter(false)),

            // StreamCreateConsumer
            () => Assert.True(Request.StreamCreateConsumer("key", "group", "consumer").Converter(1L)),
            () => Assert.False(Request.StreamCreateConsumer("key", "group", "consumer").Converter(0L)),
            () => Assert.True(Request.StreamCreateConsumer("key", "group", "consumer").Converter(true)),
            () => Assert.False(Request.StreamCreateConsumer("key", "group", "consumer").Converter(false)),

            // StreamDeleteConsumer
            () => Assert.Equal(5L, Request.StreamDeleteConsumer("key", "group", "consumer").Converter(5L)),

            // StreamConsumerGroupSetPosition
            () => Assert.True(Request.StreamConsumerGroupSetPosition("key", "group", "0-0", null).Converter("OK")),
