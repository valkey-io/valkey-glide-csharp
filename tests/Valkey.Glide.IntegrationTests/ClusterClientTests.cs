...
#pragma warning restore xUnit1046 // Avoid using TheoryDataRow arguments that are not serializable
    ...
    public async Task CustomCommandWithSingleNodeRoute(GlideClusterClient client)
    {
        ...
        res = ((await client.CustomCommand(["info", "replication"], new ByAddressRoute(TestConfiguration.CLUSTER_ADDRESSES[0].Host, TestConfiguration.CLUSTER_ADDRESSES[0].Port))).SingleValue! as gs)!;
    ...
    public async Task BatchWithSingleNodeRoute(GlideClusterClient client, bool isAtomic)
    {
        ...
        res = await client.Exec(batch, true, new(route: new ByAddressRoute(TestConfiguration.CLUSTER_ADDRESSES[0].Host, TestConfiguration.CLUSTER_ADDRESSES[0].Port)));
