...
#pragma warning restore xUnit1046 // Avoid using TheoryDataRow arguments that are not serializable
    ...
    {
        await client.SetAsync("abc", "abc");
        await client.SetAsync("klm", "klm");
        await client.SetAsync("xyz", "xyz");
    ...
    public async Task TestClientPause_WithRoute_ThenExpires(GlideClusterClient client)
    {
        ...
        // Verify that the paused node resumes once the pause expires.
    ...
    public async Task TestClusterDatabaseId()
    {
        ...
        await client.SetAsync(testKey, testValue);

        // Verify the key exists in database 1
        ValkeyValue retrievedValue = await client.GetAsync(testKey);
