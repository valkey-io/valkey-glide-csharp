...
internal partial class Database
{
        ...
        CommandFlags flags = CommandFlags.None)
    {
        ...
        var options = new MigrateOptions(host, port, toDatabase, TimeSpan.FromMilliseconds(timeoutMilliseconds))
        {
    ...
    public Task<long> SortAndStoreAsync(ValkeyKey destination, ValkeyKey key, long skip, long take, Order order, SortType sortType, ValkeyValue by, IEnumerable<ValkeyValue>? get, CommandFlags flags)
    {
        GuardClauses.ThrowIfCommandFlags(flags);
        return SortAndStoreAsync(destination, key, skip, take, order, sortType, by, get);
    }
}
