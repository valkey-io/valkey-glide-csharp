...
public abstract partial class BaseBatch<T> where T : BaseBatch<T>
{
    ...
    /// <inheritdoc cref="IBatchStringCommands.StringSet(IDictionary{ValkeyKey, ValkeyValue})" />
    public T StringSetAsync(IDictionary<ValkeyKey, ValkeyValue> values) => AddCmd(Request.StringSetMultiple([.. values]));
    ...
    IBatch IBatchStringCommands.StringSet(IDictionary<ValkeyKey, ValkeyValue> values) => StringSetAsync(values);
