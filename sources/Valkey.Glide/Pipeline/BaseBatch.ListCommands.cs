...
public abstract partial class BaseBatch<T>
{
    ...
    public T ListLeftPush(ValkeyKey key, IEnumerable<ValkeyValue> values) => AddCmd(ListLeftPushAsync(key, [.. values], When.Always));
    ...
    public T ListRightPush(ValkeyKey key, IEnumerable<ValkeyValue> values) => AddCmd(ListRightPushAsync(key, [.. values], When.Always));
    ...
    public T ListLeftPop(IEnumerable<ValkeyKey> keys, long count) => AddCmd(ListLeftPopAsync([.. keys], count));
    ...
    public T ListBlockingRightPop(IEnumerable<ValkeyKey> keys, TimeSpan timeout) => AddCmd(ListBlockingRightPopAsync([.. keys], timeout));
    ...
    public T ListBlockingPop(IEnumerable<ValkeyKey> keys, ListSide side, TimeSpan timeout) => AddCmd(ListBlockingPopAsync([.. keys], side, timeout));

    /// <inheritdoc cref="IBatchListCommands.ListBlockingPop(IEnumerable{ValkeyKey}, ListSide, long, TimeSpan)" />
    public T ListBlockingPop(IEnumerable<ValkeyKey> keys, ListSide side, long count, TimeSpan timeout) => AddCmd(ListBlockingPopAsync([.. keys], side, count, timeout));
